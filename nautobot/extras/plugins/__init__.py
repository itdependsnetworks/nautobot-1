            ...
            for extension in filter_extensions:
                if extension().filterset():
                    for name in extension().filterset().keys():
                        self.features["filter_extensions"]["filterset"].append(f"{extension.model} -> filterset -> {name}")
                if extension().filter_form():
                    for name in extension().filter_form().keys():
                        self.features["filter_extensions"]["filter_form"].append(f"{extension.model} -> filter_form -> {name}")
